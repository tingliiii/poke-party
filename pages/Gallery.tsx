    ...
    <div className="space-y-6 pb-20">
        ...
        <div className="relative z-10">
             <div className="flex items-center justify-between mb-4">
                 <div>
                    ...
                    <p className="text-slate-400 text-xs font-mono">春酒回憶錄</p>
                 </div>
         ...
         <div className="flex justify-end border-t border-white/5 pt-2 mt-4 relative z-10">
            <div className="flex bg-slate-900/80 rounded-lg p-1 border border-emerald-500/30">
                {[{ id: 'time', label: '時間', icon: Clock }, { id: 'id', label: '員編', icon: SortAsc }].map(btn => (
                  <button key={btn.id} onClick={() => handleSortChange(btn.id as any)} className={`flex items-center gap-1 px-3 py-1 rounded text-[10px] font-bold transition-all ${sortBy === btn.id ? 'bg-emerald-500 text-black' : 'text-slate-400'}`}>
                    <btn.icon size={12} />
                    {btn.label}
      ...
      {loading ? (
        <div className="grid grid-cols-3 gap-1">
            {[1,2,3,4,5,6].map(i => <div key={i} className="aspect-square bg-slate-800 animate-pulse" />)}
        </div>
      ) : (
        <div className="space-y-6">
            <div className="grid grid-cols-3 gap-1.5">
                {paginatedPhotos.map((photo, index) => {
                    ...
                    return (
                        <div key={photo.id} onClick={() => setViewingIndex(globalIndex)} className="relative aspect-square bg-slate-800 group cursor-zoom-in active:scale-95 transition-transform overflow-hidden rounded-lg border border-white/5">
                            ...
                            {/* 照片下半部資訊顯示 */}
                            <div className="absolute bottom-0 left-0 right-0 p-1.5 bg-black/60 backdrop-blur-sm border-t border-white/5 flex flex-col gap-0.5">
                                <span className="text-[8px] text-white font-mono truncate uppercase flex items-center gap-1">
                                    <User size={8} className="text-emerald-400"/> {photo.uploaderId}
                                </span>
                                <span className="text-[8px] text-slate-400 font-mono flex items-center gap-1">
                                    <Clock size={8} className="text-emerald-400"/> {formatTime(photo.timestamp)}
                                </span>
                            </div>
                            ...
                            {isOwner && (
                                <button 
                                    ...
                                    className="absolute top-1 right-1 bg-red-600/80 p-1 rounded text-white opacity-0 group-hover:opacity-100 transition-opacity z-10"
            ...
            {totalPages > 1 && (
                <div className="flex items-center justify-center gap-4 pt-4 border-t border-white/5">
                    <button 
                        ...
                        className="p-2 rounded-full bg-slate-800 border border-slate-700 text-white disabled:opacity-30 disabled:cursor-not-allowed transition-all active:scale-90"
                    >
                        <ChevronLeft size={20} />
                    </button>
                    <div className="text-xs font-mono font-bold text-emerald-400 bg-emerald-500/10 px-4 py-1.5 rounded-full border border-emerald-500/20 shadow-glow">
